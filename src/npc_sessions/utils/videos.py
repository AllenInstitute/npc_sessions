...
def get_dlc_session_paf_graph(session: str, model_name: str) -> list:
    ...
    metadata_pickle_file = tuple(path for path in model_s3_paths if path.suffix == '.pickle')
    if not metadata_pickle_file:
        raise FileNotFoundError(f'No metadata pickle found for session {session}. Check {model_name} capsule')
...
def get_dlc_session_model_dataframe_from_h5(session: str, model_name: str) -> pd.DataFrame:
    """
    ...
    if not h5_files:
        raise FileNotFoundError(f'No h5 files found for {session}. Check {model_name} capsule')
 
    df_model = h5_to_dataframe(h5_files[0])
    return df_model

def get_pose_series_from_dataframe(session: str, df: pd.DataFrame, video_timestamps: npt.NDArray[np.float64]) -> list[ndx_pose.pose.PoseEstimationSeries]:
