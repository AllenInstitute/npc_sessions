...
def get_electrode_files_from_network(session: str | npc_session.SessionRecord) -> tuple[upath.UPath, ...] | None:
    session = npc_session.SessionRecord(session)
    all_subject_sessions = npc_lims.get_subject_data_assets(session.subject)
    raw_subject_assets = sorted(tuple(asset['name'] for asset in all_subject_sessions if npc_lims.is_raw_data_asset(asset)))
    day = tuple(raw_subject_assets.index(asset) + 1 for asset in raw_subject_assets if f'{session.subject}_{session.date}' in asset)[0]
    ...
    electrode_files = tuple(subject_electrode_network_path.glob(f'Probe_*{day}_channels_{str(session.subject.id)}_warped.csv'))
...
def get_electrodes_from_network(session: str | npc_session.SessionRecord) -> pd.DataFrame | None:
    ...
    for electrode_file in electrode_files:
        ...
        probe_electrodes['device_name'] = [f'Probe{probe}' for i in range(len(probe_electrodes))]
