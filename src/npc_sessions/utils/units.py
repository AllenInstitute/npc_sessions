...
def get_units_spike_paths(
    ...
    if not records:
        ...
        if units_s3_path is not None:  
            if method == 'kilosort_codeocean':
                 return get_units_spike_paths_from_kilosort_codeocean_output(units_s3_path)

    return records

def get_spike_times(units:pd.DataFrame, spike_times_path: upath.UPath) -> dict[str, npt.NDArray[np.float64]]:
    spike_times_dict = {}
...
def get_units_spike_times(units_spike_paths: tuple[upath.UPath, ...]) -> tuple[pd.DataFrame, dict[str, npt.NDArray[np.float64]]]:
