...
class SyncDataset:
    ...
    def vsync_times_in_blocks(self) -> tuple[npt.NDArray[np.floating], ...]:
        ...
        """
        vsync_rising_edges: npt.NDArray[np.floating] = self.get_rising_edges('vsync_stim', units = 'seconds')
        vsync_falling_edges: npt.NDArray[np.floating] = self.get_falling_edges('vsync_stim', units = 'seconds')
        # ensure first vsync is rising
        vsync_falling_edges = vsync_falling_edges if vsync_rising_edges[0] < vsync_falling_edges[0] else vsync_falling_edges[1:]
        ...
        logger.info(f'Found {len(vsync_times_in_blocks)} blocks of vsync events with lengths {block_lengths}')
        
        stim_running_rising_edges = self.get_rising_edges('stim_running', units = 'seconds')
        stim_running_falling_edges = self.get_falling_edges('stim_running', units = 'seconds')
        ...
        for block_idx, (vsyncs, rising, falling) in enumerate(zip(vsync_falling_edges_in_blocks, diode_rising_edges_in_blocks, diode_falling_edges_in_blocks)):
            ...
            if round(np.mean(np.diff(diode_flips)), 1) == round(np.mean(np.diff(vsyncs)), 1):
                ...
                if len(diode_flips) != len(vsyncs):
                    logger.warning(f'Mismatch in stim {block_idx = }: {len(diode_flips) = }, {len(vsyncs) = }')
    ...
    def plot_stim_onsets(self) -> tuple[fig.Figure, list[plt.Axes]]:
        ...
        for ind, (son, vs, dvs) in enumerate(
            zip(self.stim_onsets,  self.vsync_times_in_blocks, self.frame_display_time_blocks)
        ):
            labels = []
    ...
    def plot_stim_offsets(self) -> tuple[fig.Figure, list[plt.Axes]]:
        ...
        for ind, (soff, vs, dvs) in enumerate(
            zip(self.stim_offsets, self.vsync_times_in_blocks, self.frame_display_time_blocks)
        ):
            labels = []
            axes[ind].plot(vs, 0.5 * np.ones(len(vs)), '|')
            labels.append('stim vsyncs')
    ...
    def plot_diode_measured_sync_square_flips(self) -> tuple[fig.Figure, list[plt.Axes]]:
        ...
        fig, _ = plt.subplots(1, len(stim_ons), sharey=True, gridspec_kw={'width_ratios': num_diode_flips_per_stim/min(num_diode_flips_per_stim)} )
        fig.suptitle(f'diode-measured sync-square flip intervals, {expected_period = } s')
        ...
        for idx, (ax, d) in enumerate(zip(fig.axes, diode_flips_per_stim)):
            ...
            ax.axhline(expected_period, linewidth=.5, c='k', linestyle='--', alpha=0.3)
            plt.sca(ax)
            intervals = np.diff(d)
            times = np.diff(d) / 2 + d[:-1] # plot at mid-point of interval
            markerline, stemline, baseline = plt.stem(times, intervals, bottom=expected_period)
