...
Interval: TypeAlias = Union[Sequence[float], Mapping[Literal['start_time', 'stop_time'], float], pd.Series, pd.DataFrame]
...
def is_valid_interval(session, interval: Interval) -> bool:
    ...
    if session.invalid_times is not None:
        if any(is_overlap_in_intervals(interval, invalid_interval) for invalid_interval in parse_intervals(session.invalid_times)):
            return False
    return True
...
def parse_units(session, unit_selection: UnitSelection = None) -> pd.DataFrame:
    ...
    elif isinstance(unit_selection, (int, Iterable)): # beware order: str/DataFrame are Iterable
        units = session.units[unit_selection]
    elif unit_selection is None:
        units = session.units[:]
    else:
        raise TypeError(f"Expected unit_selection to be {UnitSelection=} - got {unit_selection!r}")
...
def get_spike_counts(
    ...
    counts = np.diff(np.searchsorted(spike_times, intervals)) # axis not supported in np.diff for numba
    return counts
...
def apply_invalid_intervals(
    ...
    for idx, interval in enumerate(_intervals):
        if (
            not is_valid_interval(session, interval)
            or (
...
def get_spike_counts_in_intervals(
    ...
    for unit in rich.progress.track(
        ...
        spikes_per_interval = get_spike_counts(np.array(unit.spike_times), np.array(_intervals))
        if as_spikes_per_second:
            spikes_per_interval / np.diff(_intervals) 
        _spikes_per_interval_per_unit.append(spikes_per_interval)
        
    spikes_per_interval_per_unit = apply_invalid_intervals(session, _intervals, _spikes_per_interval_per_unit, unit_selection)
...
def get_response_in_intervals(
    ...
    if not as_spikes_per_second:
        assert np.diff(parse_intervals(response_intervals)) == np.diff(parse_intervals(baseline_intervals)), "response and baseline intervals must have same duration to express response as spike counts"
    _response = get_spike_counts_in_intervals(session, response_intervals, unit_selection, as_spikes_per_second)
    _baseline = get_spike_counts_in_intervals(session, baseline_intervals, unit_selection, as_spikes_per_second) 
            ...
            stim_by_unit_responses.append(
                np.nanmean(
                    get_response_in_intervals(
                        ...
                        as_spikes_per_second=True,
                        as_normalized_ratio=False,
                        unit_selection=session.units[:], # slight speed-up by creating df only once
                    ),
                    axis=1,
                )
            )
