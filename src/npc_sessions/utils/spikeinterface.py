...
class SpikeInterfaceKS25Data:
    ...
    def get_path(self, dirname: str, probe: str | None) -> upath.UPath:
        ...
        if probe is None:
            ...
            return next(path for path in self.format_path(self.root, dirname).iterdir() if npc_session.ProbeRecord(probe).lower() in path.as_posix().lower())
    ...
    visualization_output_json = functools.partialmethod(get_json, "visualization_output.json")
    ...
    def quality_metrics_dict(self, probe: str) -> dict:
        return self.read_json(self.format_path(self.postprocessed(probe), "quality_metrics", "params.json"))
    
    @functools.cache
    def quality_metrics_df(self, probe: str) -> pd.DataFrame:
        return self.read_csv(self.format_path(self.postprocessed(probe), "quality_metrics", "metrics.csv"))
    
    @functools.cache
    def template_metrics_dict(self, probe: str) -> dict:
        return self.read_json(self.format_path(self.postprocessed(probe), "template_metrics", "params.json"))
