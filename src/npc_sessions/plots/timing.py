...
def plot_bad_lick_times(session: 'npc_sessions.DynamicRoutingSession') -> None:
    ...
    trials_with_lick_outside_response_window = (
        session.trials[:].query("is_response")
        .query("response_window_start_time > response_time or response_window_stop_time < response_time")
        ).index.tolist()

    trials_with_lick_inside_response_window_but_not_recorded = (
        session.trials[:].query("not is_response")
        .query("response_window_start_time <= response_time <= response_window_stop_time")
        ).index


    for idx in (*trials_with_lick_outside_response_window, *trials_with_lick_inside_response_window_but_not_recorded):
        ...
        padding = 0.3
        marker_config = dict(linestyles="-", linelengths=0.2)
        line_config = dict(linestyles="-", linelengths=1)
        plt.eventplot(
            vsyncs[(vsyncs >= start - padding) & (vsyncs <= stop + padding)], **line_config,
            label="vsyncs", alpha=.5, color="orange", 
            )
        plt.eventplot(
            vsyncs[(vsyncs >= start) & (vsyncs <= stop)], **line_config,
            label="vsyncs within response window", color="orange",
            )
        plt.eventplot(
            licks[(licks >= start - padding) & (licks <= stop + padding)], **marker_config, 
            label="licks", color="k", lineoffsets=1
            )
        plt.eventplot(
            [session._trials.get_script_frame_time(session._trials._sam.trialResponseFrame[idx])], **marker_config,
            label="lick frame in TaskControl", color="lime", lineoffsets=1.6
            )
        plt.eventplot(
            [session._trials.get_script_frame_time(session._trials._sam.stimStartFrame[idx])], **marker_config,
            label="stim start frame in TaskControl", color="r", lineoffsets=1.6
            )
...
def plot_lick_times_on_sync_and_script(session: 'npc_sessions.DynamicRoutingSession') -> None:
    ...
    script_time = session._trials.get_script_frame_time(session._trials._sam.trialResponseFrame)
