...
def plot_video_info(
    ...
    augmented_camera_info = utils.get_augmented_camera_info(session.sync_data, *session.video_paths)

    for camera, info in augmented_camera_info.items():
        rich.print(f'[bold]{camera} camera stats[bold]')
        
        frame_rate = info['FPS']
        frame_rate_string = plot_utils.add_valence_to_string(f'Frame Rate: {frame_rate} \t',
                                                    frame_rate,
                                                    abs(frame_rate-60)<0.01, 
                                                    abs(frame_rate-60)>0.05)
        
        lost_frame_percentage = 100 * info['FramesLostCount'] / info['FramesRecorded']
        lost_frame_string = plot_utils.add_valence_to_string(f'Lost frame percentage: {np.round(lost_frame_percentage, 3)} \t',
        ...
        frame_diff_string = plot_utils.add_valence_to_string(f'Frames expected minus actual: {frame_diff_from_expected}',
...
def plot_video_frames_with_licks(
    ...
    response_times: npt.NDArray = session.trials[:].query('is_response').response_time.to_numpy()
    lick_times = sorted(random.sample(tuple(response_times[~np.isnan(response_times)]), NUM_LICKS))
    
    fig = plt.figure(figsize=[12, 5 * NUM_LICKS],  facecolor='0.5')
    # fig, axes = plt.subplots(NUM_LICKS * ROWS_PER_LICK, FRAMES_PER_ROW,)
    # constrained_layout=True,
    gs = gridspec.GridSpec(NUM_LICKS * ROWS_PER_LICK, FRAMES_PER_ROW, height_ratios=[1, 0.1] * NUM_CAMERAS * NUM_LICKS)
    gs.update(wspace=0.0, hspace=0.0)
    
    video_frame_times = utils.get_video_frame_times(session.sync_data, *session.video_paths)
    for vid_idx, (video_path, frame_times) in enumerate(video_frame_times.items()):
        ...
        for lick_idx, lick_time in enumerate(lick_times):
            ...
            closest_frame_index = np.nanargmin(np.abs(frame_times - lick_time))
            frame_indices = np.arange(closest_frame_index - FRAMES_EITHER_SIDE_OF_LICK, closest_frame_index + FRAMES_EITHER_SIDE_OF_LICK + 1)
            ...
            for frame_idx, frame_index in enumerate(frame_indices):
                ...
                ret, frame = v.read()
                ax = fig.add_subplot(gs[(vid_idx*ROWS_PER_VIDEO) + (lick_idx * ROWS_PER_LICK), frame_idx])
                ...
                if frame_index == closest_frame_index:
                    trial = np.searchsorted(session.trials[:].start_time, lick_time, 'right') -1
                    ax.set_title(f'{trial=}, {lick_time=:.1f} s', fontsize=8)
                    
            # markers for frame and lick times
            ax = fig.add_subplot(gs[1 + (vid_idx*ROWS_PER_VIDEO) + (lick_idx * ROWS_PER_LICK), :FRAMES_PER_ROW])
            linekwargs = dict(linewidths=3) 
