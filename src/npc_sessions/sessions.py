...
class DynamicRoutingSession:
    ...
    def nwb_from_cache(self) -> pynwb.NWBFile | None:
        if (
            path := npc_lims.get_nwb_path(self.id, version=utils.get_package_version())
        ).exists():
            if path.suffix == ".zarr":
    ...
    def write_nwb_hdf5(
        self, path: str | npc_io.PathLike | None = None, metadata_only: bool = False
    ) -> upath.UPath:
