...
class DynamicRoutingSession:
    ...
    def keywords(self) -> list[str]:
        if getattr(self, "_keywords", None) is None:
            ...
            if self.is_task and self.is_first_block_aud:
                self.keywords.append("first_block_aud")
            elif self.is_task and self.is_first_block_vis:
                self.keywords.append("first_block_vis")
    ...
    def _all_trials(self) -> npc_io.LazyDict[str, TaskControl.TaskControl]:
        ...
        def get_intervals(
            ...
            logger.info(f"Generating intervals: {cls.__name__}({stim_path.stem!r})")
            stim_path = next(path for path in stim_paths if path.stem == stim_path.stem)
    ...
    @property
    def electrode_channel_idx_offset(self) -> int:
    ...
    def _units(self) -> pd.DataFrame:
        ...
        for electrode_group_name in units["electrode_group_name"].unique():
            group_to_obs_intervals[electrode_group_name] = self.get_obs_intervals(
                electrode_group_name
            )
        units["obs_intervals"] = units["electrode_group_name"].map(
            group_to_obs_intervals
        )
        if self.is_annotated:
    ...
    def units(self) -> pynwb.misc.Units:
        ...
        for _, row in self._units.iterrows():
            ...
            if len(electrodes.query(group_query)) == 0:
                raise IndexError(
                    f"No electrodes found matching {group_query} - annotations may be missing"
                )
    ...
    @property
    def is_first_block_aud(self) -> bool:
        if not self.is_task:
            raise AttributeError(f"{self.id} is not a session with behavior task")
        return "sound" in self.sam.blockStimRewarded[0]

    @property
    def is_first_block_vis(self) -> bool:
        if not self.is_task:
            raise AttributeError(f"{self.id} is not a session with behavior task")
        return "vis" in self.sam.blockStimRewarded[0]
    ...
    def electrode_group_description(self) -> str:
        return f"Neuropixels 1.0 ({1 + self.electrode_channel_idx_offset}:{384 + self.electrode_channel_idx_offset})"
