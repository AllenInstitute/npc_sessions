...
def get_max_workers() -> int:
    return min(psutil.cpu_count(), psutil.virtual_memory().available * 0.7 / MEMORY_PER_SESSION)

def setup() -> dict[Literal["session_type", "skip_existing", "version", "parallel"], str | bool | None]:
    args = parse_args()
    kwargs = vars(args)
    logging.basicConfig(
...
def get_session_infos(session_type: Literal["training", "ephys", "all"]) -> tuple[npc_lims.SessionInfo, ...]:
    ...
    if session_type == "all":
        ...
        session_infos = tuple(
            s for s in npc_lims.get_session_info() if s.is_ephys == is_ephys(session_type=session_type)
        )
