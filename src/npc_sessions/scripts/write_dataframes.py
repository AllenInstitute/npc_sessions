...
def get_all_ephys_session_dfs(**session_kwargs) -> dict[str, pd.DataFrame]:
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        for idx, session in enumerate(npc_sessions.get_sessions()): # only tracked-ephys sessions
            if not (session.is_sorted and session.is_annotated):
                continue
            session_to_future[session.id] = executor.submit(get_session_dfs, session.id, attrs, **session_kwargs)
    for future in concurrent.futures.as_completed(session_to_future.values()):
        with contextlib.suppress(Exception):
            session_dfs = future.result()
            for attr in attrs:
                all_session_dfs[attr] = pd.concat((all_session_dfs[attr], session_dfs[attr]))
