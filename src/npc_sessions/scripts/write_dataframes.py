...
def get_session_dfs(session: str | npc_session.SessionRecord | npc_sessions.DynamicRoutingSession, attrs: Iterable[str], **session_kwargs) -> dict[str, pd.DataFrame]:
    ...
    for attr in attrs:
        ...
        if hasattr(container, "to_dataframe"):
            ...
            if attr == "units":
                for col in ("electrodes", "electrode_group", "waveform_sd", "waveform_mean"):
...
def get_all_ephys_session_dfs(**session_kwargs) -> dict[str, pd.DataFrame]:
    ...
    future_to_session_id: dict[concurrent.futures.Future, npc_session.SessionRecord] = {}
    ...
    for idx, session in enumerate(npc_sessions.get_sessions()):
        if not (session.is_sorted and session.is_annotated):
            continue
        print(idx)
        future = loky.get_reusable_executor(max_workers=4).submit(get_session_dfs, session.id, attrs, **session_kwargs)
...
def write_df(path: npc_sessions.PathLike, df: pd.DataFrame, append: bool = False) -> None:
