...
def write_training_sessions_to_cache(
    ...
    for session_info in tqdm.tqdm(npc_lims.get_session_info(is_ephys=False), desc='Submitting jobs'):
        future_to_session[
            pool.submit(
                functools.partial(npc_sessions.write_all_components_to_cache, skip_existing=skip_existing),
                npc_sessions.DynamicRoutingSession(session_info)
            )
        ] = session_info.id
    for future in tqdm.tqdm(concurrent.futures.as_completed(future_to_session.keys()), desc='Processing jobs', total=len(future_to_session)):
